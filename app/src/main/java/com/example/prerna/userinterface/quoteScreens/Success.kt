...
@Composable
fun SuccessScreen(
    successQuotesList: List<quote>,
    navController: NavController,
    favoriteViewModel: FavoriteViewModel,
    userAddedFavoriteViewModel: UserAddedFavoriteViewModel
) {
    var successQuotes = userAddedFavoriteViewModel.successQuotes
    Scaffold(
        modifier = Modifier.fillMaxSize(),
        topBar = {
            TopScreenBar(R.string.success, navController = navController)
        },
        content = { paddingValues ->
            LazyColumn(contentPadding = paddingValues) {
                // Items from the first list (bhajanList)
                itemsIndexed(successQuotesList) { index, quote ->
                ...
                itemsIndexed(successQuotes) { index, quote ->
                    UserAddedQuoteCard(dustBin = true, quote, userAddedFavoriteViewModel, onDelete = {userAddedFavoriteViewModel.deleteQuote("success", quote)})
                }
            }
        }
    )
