...
@Composable
fun WisdomScreen(
    wisdomQuotesList: List<quote>,
    navController: NavController,
    favoriteViewModel: FavoriteViewModel,
    userAddedFavoriteViewModel: UserAddedFavoriteViewModel
) {
    var wisdomQuotes = userAddedFavoriteViewModel.wisdomQuotes
    Scaffold(
        modifier = Modifier.fillMaxSize(),
        topBar = {
            TopScreenBar(R.string.wisdom, navController = navController)
        },
        content = { paddingValues ->
            LazyColumn(contentPadding = paddingValues) {
                // Items from the first list (bhajanList)
                itemsIndexed(wisdomQuotesList) { index, quote ->
                    QuoteCard(quote.quoteResId, favoriteViewModel)
                }

                if(!wisdomQuotes.isEmpty()) {
                    item {
                        Row(
                            modifier = Modifier
                                .fillMaxWidth()
                ...
                itemsIndexed(wisdomQuotes) { index, quote ->
                    UserAddedQuoteCard(dustBin = true, quote, userAddedFavoriteViewModel, onDelete = {userAddedFavoriteViewModel.deleteQuote("wisdom", quote)})
                }
            }
        }
    )
