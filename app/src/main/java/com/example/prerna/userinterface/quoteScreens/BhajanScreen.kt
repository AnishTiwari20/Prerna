...
@Composable
fun BhajanScreen(
    bhajanList: List<quote>,
    navController: NavController,
    favoriteViewModel: FavoriteViewModel,
    userAddedFavoriteViewModel: UserAddedFavoriteViewModel
) {

    val bhajanQuotes = userAddedFavoriteViewModel.bhajanQuotes
    Scaffold(
        modifier = Modifier.fillMaxSize(),
        topBar = {
            TopScreenBar(R.string.bhajan, navController = navController)
        },
        content = { paddingValues ->
            LazyColumn(contentPadding = paddingValues) {
                // Items from the first list (bhajanList)
                itemsIndexed(bhajanList) { index, quote ->
                    QuoteCard(quote.quoteResId, favoriteViewModel)
                }

                if(!bhajanQuotes.isEmpty()) {
                    item {
                        Row(
                            modifier = Modifier
                                .fillMaxWidth()
                ...
                itemsIndexed(bhajanQuotes) { index, quote ->
                    UserAddedQuoteCard(dustBin = true, quote, userAddedFavoriteViewModel, onDelete = {userAddedFavoriteViewModel.deleteQuote("bhajan", quote)})
                }
            }
        }
