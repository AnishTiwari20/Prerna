...
@Composable
fun LoveScreen(
    loveQuotes: List<quote>,
    navController: NavController,
    favoriteViewModel: FavoriteViewModel,
    userAddedFavoriteViewModel: UserAddedFavoriteViewModel
) {
    var loveQuote = userAddedFavoriteViewModel.loveQuotes
    Scaffold(
        modifier = Modifier.fillMaxSize(),
        topBar = {
            TopScreenBar(R.string.love, navController = navController)
        },
        content = { paddingValues ->
            LazyColumn(contentPadding = paddingValues) {
                // Items from the first list (bhajanList)
                itemsIndexed(loveQuotes) { index, quote ->
                    QuoteCard(quote.quoteResId, favoriteViewModel)
                }

                if(!loveQuote.isEmpty()) {
                    item {
                        Row(
                            modifier = Modifier
                                .fillMaxWidth()
                ...
                itemsIndexed(loveQuote) { index, quote ->
                    UserAddedQuoteCard(dustBin = true, quote, userAddedFavoriteViewModel, onDelete = {userAddedFavoriteViewModel.deleteQuote("love", quote)})
                }
            }
        }
    )
