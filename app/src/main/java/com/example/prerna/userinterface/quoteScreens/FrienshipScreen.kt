...
fun FriendshipScreen(
    friendshipquotesList: List<quote>,
    navController: NavController,
    favoriteViewModel: FavoriteViewModel,
    userAddedFavoriteViewModel: UserAddedFavoriteViewModel
) {
    val friendShipQuotes = userAddedFavoriteViewModel.friendshipQuotes
    Scaffold(
        modifier = Modifier.fillMaxSize(),
        topBar = {
            TopScreenBar(R.string.friendship, navController = navController)
        },
        content = { paddingValues ->
            LazyColumn(contentPadding = paddingValues) {
                // Items from the first list (bhajanList)
                itemsIndexed(friendshipquotesList) { index, quote ->
                    QuoteCard(quote.quoteResId, favoriteViewModel)
                }

                if(!friendShipQuotes.isEmpty()) {
                    item {
                        Row(
                            modifier = Modifier
                                .fillMaxWidth()
                ...
                itemsIndexed(friendShipQuotes) { index, quote ->
                    UserAddedQuoteCard(dustBin = true, quote, userAddedFavoriteViewModel, onDelete = {userAddedFavoriteViewModel.deleteQuote("friendship", quote)})
                }
            }
        }
    )
